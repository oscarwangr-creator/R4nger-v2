...
class GeneratedModule(JsonEntityModule):
    name = "securitytrails_module"
    category = "generated"
    input_types = ['seed']
    output_types = ['artifact']
    command_template = 'securitytrails {seed}'
            ...
            'artifacts': [
                {
                    ...
                    'toolchain': ['securitytrails'],
                    'pipelines': ['organization_profiling_pipeline', 'supply_chain_risk_pipeline', 'vendor_attack_surface_pipeline'],
                    'confidence': 0.7,
                }
            ],
        }
