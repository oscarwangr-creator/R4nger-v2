...
class GeneratedModule(JsonEntityModule):
    name = "osintcombine_module"
    category = "generated"
    input_types = ['seed']
    output_types = ['artifact']
    command_template = 'osintcombine {seed}'
            ...
            'artifacts': [
                {
                    ...
                    'toolchain': ['osintcombine'],
                    'pipelines': ['travel_pattern_pipeline', 'certificate_transparency_pipeline', 'legal_exposure_pipeline', 'api_surface_pipeline'],
                    'confidence': 0.7,
                }
            ],
        }
