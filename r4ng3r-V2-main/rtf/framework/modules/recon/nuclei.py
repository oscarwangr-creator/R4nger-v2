...
class NucleiModule(ExternalToolModule):
    tool_name = "nuclei"
    tool_description = "Run nuclei from the CLI and parse JSON line output."
    tool_category = "recon"
    tool_binary = "nuclei"
    tool_refs = ['https://github.com/projectdiscovery/nuclei']
    target_option_name = "target"
    target_option_desc = "Target URL, host, or file path"
    default_timeout = 600
    ...
    def build_command(self, target: str, output_file: str) -> Tuple[List[str], str | None]:
        ...
        cmd.extend(["-u", str(target), "-json-export", json_path, "-silent", "-no-color"])
