...
class HttpxModule(ExternalToolModule):
    tool_name = "httpx"
    tool_description = "Run httpx from the CLI and parse JSON line output."
    tool_category = "recon"
    tool_binary = "httpx"
    tool_refs = ['https://github.com/projectdiscovery/httpx']
    target_option_name = "target"
    target_option_desc = "Target host, URL, or file path"
    default_timeout = 600
    ...
    def build_command(self, target: str, output_file: str) -> Tuple[List[str], str | None]:
        ...
        temp_output = None
        cmd.extend(["-u", str(target), "-json", "-silent", "-no-color"])
