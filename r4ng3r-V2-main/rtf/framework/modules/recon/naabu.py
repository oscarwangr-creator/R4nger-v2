...
class NaabuModule(ExternalToolModule):
    tool_name = "naabu"
    tool_description = "Run naabu from the CLI and parse JSON line output."
    tool_category = "recon"
    tool_binary = "naabu"
    tool_refs = ['https://github.com/projectdiscovery/naabu']
    target_option_name = "target"
    target_option_desc = "Target host or CIDR"
    default_timeout = 600
    ...
    def build_command(self, target: str, output_file: str) -> Tuple[List[str], str | None]:
        ...
        temp_output = None
        cmd.extend(["-host", str(target), "-json"])
        if limit:
            cmd.extend(["-top-ports", str(limit)])
