...
class AmassModule(ExternalToolModule):
    tool_name = "amass"
    tool_description = "Run amass from the CLI and return normalized line-oriented results."
    tool_category = "recon"
    tool_binary = "amass"
    tool_refs = ['https://github.com/owasp-amass/amass']
    ...
    def build_command(self, target: str, output_file: str) -> Tuple[List[str], str | None]:
        ...
        temp_output = None
        cmd.extend(["enum", "-d", str(target), "-norecursive", "-noalts"])
