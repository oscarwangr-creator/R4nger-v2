...
class TruffleHogModule(ExternalToolModule):
    tool_name = "trufflehog"
    tool_description = "Run trufflehog from the CLI and parse JSON line output."
    tool_category = "osint"
    tool_binary = "trufflehog"
    tool_refs = ['https://github.com/trufflesecurity/trufflehog']
    target_option_name = "target"
    target_option_desc = "Git repository, filesystem path, or URI"
    default_timeout = 600
    ...
    def build_command(self, target: str, output_file: str) -> Tuple[List[str], str | None]:
        ...
        temp_output = None
        if str(target).startswith(("http://", "https://", "git@")):
            cmd.extend(["git", str(target), "--json"] )
        else:
            cmd.extend(["filesystem", str(target), "--json"])
