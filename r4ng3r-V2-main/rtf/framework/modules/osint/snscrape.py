...
class SnscrapeModule(ExternalToolModule):
    tool_name = "snscrape"
    tool_description = "Run snscrape from the CLI and parse JSON line output."
    tool_category = "osint"
    tool_binary = "snscrape"
    tool_refs = ['https://github.com/JustAnotherArchivist/snscrape']
    target_option_name = "query"
    target_option_desc = "snscrape query or username"
    default_timeout = 600
    ...
    def build_command(self, target: str, output_file: str) -> Tuple[List[str], str | None]:
        ...
        temp_output = None
        mode = mode or "twitter-search"
        cmd.extend(["--jsonl", f"{mode}:{target}"])
