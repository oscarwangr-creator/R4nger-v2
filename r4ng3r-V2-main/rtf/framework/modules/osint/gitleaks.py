...
class GitleaksModule(ExternalToolModule):
    tool_name = "gitleaks"
    tool_description = "Run gitleaks from the CLI and parse JSON output."
    tool_category = "osint"
    tool_binary = "gitleaks"
    tool_refs = ['https://github.com/gitleaks/gitleaks']
    target_option_name = "target"
    target_option_desc = "Git repository or filesystem path"
    default_timeout = 600
    ...
    def build_command(self, target: str, output_file: str) -> Tuple[List[str], str | None]:
        ...
        cmd.extend(["detect", "--source", str(target), "--report-format", "json", "--report-path", json_path])
